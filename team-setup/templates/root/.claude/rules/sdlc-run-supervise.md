...
- Launch with **`--supervise --detach`** and **`--heartbeat`** (see #38 / #39).
- **Do not** block the agent turn on multi-minute poll/`sleep` loops while
  waiting for sandbox, CI, or implementation agents.
- After launch: confirm alive + first heartbeat/monitor line, report once,
  yield; check in later via `/loop` or heartbeat/log wakes.
