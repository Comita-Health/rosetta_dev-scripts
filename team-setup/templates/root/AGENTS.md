...
| Slash-style prompts  | `.claude/commands/` (`/review`, `/add-repo`, `/sdlc-status`) | Matching `.cursor/rules/command-*.mdc` — ask the agent to follow them  |
| Agent skills         | `.claude/skills/`                                            | `.cursor/skills/` (e.g. `sdlc-prd-progress` for shadow run scorecards) |
