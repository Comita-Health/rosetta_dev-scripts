...
eval "$(bash ~/.config/comita/github-app-activate.sh)"  # addi-m[bot]
...
bunx tsx src/index.ts drop \
  ...
  --repo "$REPO" \
  --issues "$ISSUE" \
  --base-ref origin/build-env/dev \
  --require-approve
# admissions default branch is build-env/dev; docs/website/this fork use origin/main
# --mode direct|bug-spec|plan-artifact  (default: direct)
```

**Comita:** activate `~/.config/comita/github-app-activate.sh`. Product
PRs that must wait for Approve use `--require-approve` (`ADDI_MERGE_ON_APPROVE`
is on for admissions). This `rosetta_dev-scripts` clone is a fork — push
to `origin` and `gh pr create --repo Comita-Health/rosetta_dev-scripts`.
